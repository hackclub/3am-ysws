...
export default async function HomePage() {
  const stats = { makers: 0, projects: 0, approvedProjects: 0, hours: 0 };
  const config = await getYswsConfig();
  const submissionsOpen = submissionsAreOpen(config);
    ...
    <>
      <section className={styles.hero}>
        ...
        <div className={styles.actions}>
          {submissionsOpen ? <ButtonLink href="/login">start building</ButtonLink> : <ButtonLink href="#how-it-works">view the event</ButtonLink>}
          <ButtonLink href="/#how-it-works" variant="ghost">how it works</ButtonLink>
        </div>
        <div className={styles.stats} aria-label="3am stats">
          <div className={styles.stat}><strong>{stats.makers.toLocaleString()}</strong><span>makers</span></div>
          <div className={styles.stat}><strong>{stats.projects.toLocaleString()}</strong><span>projects</span></div>
          <div className={styles.stat}><strong>{stats.approvedProjects.toLocaleString()}</strong><span>approved</span></div>
          <div className={styles.stat}><strong>{stats.hours.toLocaleString()}h</strong><span>hours approved</span></div>
        </div>
      ...
      <Section id="how-it-works" label="how it works"><Steps /></Section>
      <Section id="faq" label="questions"><Faq /></Section>
    </>
  );
}
